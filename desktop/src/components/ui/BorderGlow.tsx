...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  const borderOpacity = isVisible
    ? Math.max(0, (edgeProximity * 100 - colorSensitivity) / (100 - colorSensitivity)) * glowIntensity
    : 0;
  const glowOpacity = isVisible
    ? Math.max(0, (edgeProximity * 100 - edgeSensitivity) / (100 - edgeSensitivity)) * glowIntensity
    : 0;
  ...
  const useDirectionalMask = coneSpread > 0 && coneSpread < 50;
  const borderMask = useDirectionalMask
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          maskImage: [
            ...
            ...(useDirectionalMask ? [`conic-gradient(from ${angleDeg} at center, transparent 5%, black 15%, black 85%, transparent 95%)`] : []),
          ].join(', '),
          WebkitMaskImage: [
            'linear-gradient(to bottom, black, black)',
            'radial-gradient(ellipse at 50% 50%, black 40%, transparent 65%)',
      ...
      <span
        ...
        style={{
          inset: `${-glowRadius}px`,
          maskImage: useDirectionalMask ? `conic-gradient(from ${angleDeg} at center, black 2.5%, transparent 10%, transparent 90%, black 97.5%)` : undefined,
          WebkitMaskImage: useDirectionalMask ? `conic-gradient(from ${angleDeg} at center, black 2.5%, transparent 10%, transparent 90%, black 97.5%)` : undefined,
          opacity: glowOpacity,
